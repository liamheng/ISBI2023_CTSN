...
def train_step(train_loader, model, epoch, optimizer, criterion, args):
    ...
    for step, (imagesA, imagesB, imagesC, labels) in enumerate(train_loader):
        ...
        out_A, out_B, out_C, out_F, combine,_ = model(imagesA, imagesB, imagesC)
...
def save_output(label_test_file, dataPRED, args, save_file):  # dataPRED结果向量
    label_list = args.label_idx  # label_idx=['Good', 'Usable', 'Reject']
    n_class = len(label_list)
    datanpPRED = np.squeeze(dataPRED.cpu().numpy()) # 去除维度为1的地方
    ...
    #     result[label_list[i]] = datanpPRED[:, i]


    # print("result:",result)
    result['image_name'] = image_names
    out_df = pd.DataFrame(result) # DataFrame是由多种类型的列构成的二维标签数据结构
    # print("out_df:",out_df)
    name_older = ['image_name']
    # 构造title,name_older = ['image_name','Good', 'Usable', 'Reject']
    for i in range(n_class):
