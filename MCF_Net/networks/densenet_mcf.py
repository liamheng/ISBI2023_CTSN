...
class dense121_mcs(nn.Module):
    ...
    def forward(self, x, y, z):
        ...
        combine1 = self.combine2(combine)   # torch.Size([4, 2])

        # Prediction - level
        combine3 = torch.cat((x1.view(x1.size(0), -1),
                              y1.view(y1.size(0), -1),
                              z1.view(z1.size(0), -1),
                              combine1.view(combine1.size(0), -1)), 1) # torch.Size([4, 8])
        ...
        return x1, y1, z1, combine1, combine3 ,combine  # out_A, out_B, out_C, out_F, combine ,cat（三个特征）
